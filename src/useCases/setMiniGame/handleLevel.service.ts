...
export class HandleLevel {
  ...
  async getNewCaraminhola(id: string) {
    ...
    const caraminholaRelation = caraminhola?.Caraminhola_relation.map(
      (relation) => {
        return relation.caraminhola_id;
      }
    );
    if (!caraminholaRelation?.length)
      throw new Error("Caraminhola Relation not found");

    const caraminholaRelationShuffle = this.shuffleArray(caraminholaRelation);


    const caraminholaRelationCardsSelect =
      await this.caraminholaRepository.findMany({
        where: {
          id: {
